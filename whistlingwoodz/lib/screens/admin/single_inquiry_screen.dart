...
class SingleInquiry extends StatefulWidget {
  const SingleInquiry({super.key});

  @override
  State<SingleInquiry> createState() => _SingleInquiryState();
}

class _SingleInquiryState extends State<SingleInquiry>
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: [
            if (_selectedIndex == 0) const DashboardPage(),
            if (_selectedIndex == 1) const InquiryList(),
            if (_selectedIndex == 2) const AdminSetting(),
          ],
        ),
      ),
    );
